...
// Define the types for the data being fetched
interface Banner {
  id: number;
  image: string;
}
...
const Home: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"cgv" | "promotion">("cgv"); // Default to "cgv"
  const navigate = useNavigate();
  const { t } = useTranslation(); // Use the translation hook

  const menuItems = [
    { className: "bg-menu-1", position: "10px 0" },
    { className: "bg-menu-2", position: "-140px 0" },
    { className: "bg-menu-3", position: "-280px 0" },
    { className: "bg-menu-4", position: "-420px 0" },
    { className: "bg-menu-5", position: "-560px 0" },
    { className: "bg-menu-6", position: "-700px 0" },
    { className: "bg-menu-7", position: "-830px 0" },
  ];

  const eventTabs = [
    {
      text: t("home.cgv_members"), // Use t() for translation
      icon: "https://www.cgv.vn/skin/frontend/cgv/default/images/ico_finger.png",
      id: "cgv",
    },
    {
      text: t("home.new_promotion"), // Use t() for translation
      icon: "https://www.cgv.vn/skin/frontend/cgv/default/images/ico_finger.png",
      id: "promotion",
    },
  ];
            ...
            {menuItems.map((menuItem, index) => (
              <li key={index}>
                <a
                  ...
                  style={{ backgroundPosition: menuItem.position }}
                ></a>
              </li>
            ))}
          </ul>
                  ...
                  <div className="relative w-[240px] h-[355px] overflow-hidden flex justify-center items-center">
                    ...
                    <div className="absolute bottom-0 left-0 z-[999] w-full text-white font-bold text-[15px] py-[10px] bg-[url('/public/images/bg-black-transparent.png')] opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      ...
                      <div className="title-content flex justify-evenly items-center mt-10">
                        ...
                        <button className="bg-red-700 text-white py-[7px] px-[8px] rounded-[5px] text-[13px] flex items-center hover:bg-red-900 cursor-pointer">
                          {t("home.buy_ticket")}
                        </button>
                      </div>
                    </div>
            ...
            {eventTabs.map((tab) => (
              <li
                ...
                className={`inline-block bg-[#e71a0f] h-[40px] leading-[40px] text-white cursor-pointer bg-no-repeat ${
                  tab.id === "cgv"
                    ? "bg-[url('https://www.cgv.vn/skin/frontend/cgv/default/images/ribon_left_menu.gif')] bg-left px-[10px] pl-[25px]"
                    : "bg-[url('https://www.cgv.vn/skin/frontend/cgv/default/images/ribon_right.gif')] bg-right px-[10px] pr-[25px]"
                }`}
